...
const AIChat: React.FC = () => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    { role: 'model', text: 'Olá! Sou o Titan Copilot 🐟. Como posso ajudar com seu aquário hoje?' }
  ]);
  ...
  return (
    <div className="fixed bottom-4 right-4 md:bottom-6 md:right-6 z-50 flex flex-col items-end pointer-events-auto">
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="mb-4 w-[90vw] md:w-96 bg-black/80 backdrop-blur-xl border border-white/10 rounded-2xl overflow-hidden shadow-2xl shadow-[#4fb7b3]/20"
          >
            {/* Header */}
            <div className="bg-gradient-to-r from-[#1a1b3b] to-[#31326f] p-4 flex justify-between items-center border-b border-white/10">
              <div className="flex items-center gap-2">
                <Sparkles className="w-5 h-5 text-[#4fb7b3] animate-pulse" />
                <h3 className="font-heading font-bold text-white tracking-wider">Titan Copilot</h3>
              </div>
                ...
                >
                  <div
                    className={`max-w-[85%] p-3 rounded-lg text-sm ${
                      msg.role === 'user'
                        ? 'bg-[#4fb7b3] text-black rounded-tr-none'
            ...
            <div className="p-3 border-t border-white/10 bg-black/40">
              <div className="flex gap-2">
                <input
                  ...
                  placeholder="Pergunte sobre parâmetros da água..."
                  className="flex-1 bg-transparent text-white placeholder-white/30 text-sm focus:outline-none"
                />
                <button
                  onClick={handleSend}
                  disabled={isLoading || !input.trim()}
                  className="bg-[#4fb7b3] p-2 rounded-lg hover:bg-[#a8fbd3] transition-colors disabled:opacity-50"
                  data-hover="true"
                >
      ...
      <motion.button
        ...
        className="w-12 h-12 md:w-14 md:h-14 rounded-full bg-gradient-to-tr from-[#4fb7b3] to-[#31326f] flex items-center justify-center shadow-lg shadow-[#4fb7b3]/40 border border-white/20 z-50 group"
        data-hover="true"
      >
        {isOpen ? (
